...
std::unique_ptr<SearchResults>
    Solver_BFS::Solve(u_char size, steady_clock::time_point &clock_start)
{
...
#pragma region Search State Space
    ...
    for (long i = 0; i < layers || layers < 0; i++)
    {
        while (!frontier->empty())
        {
            ...
            {
                // Generate children
                auto boardChildren = frontierBoard->GenChildBoards(true, true);
...
#pragma endregion
    ...
    return std::move(std::unique_ptr<SearchResults>(
        new SearchResults(explored->size(), expectedBoards,
                          std::move(solutions), GetTime(clock_start))));
}

std::string Solver_BFS::GetName() { return "Row-Only Breadth First Search"; }