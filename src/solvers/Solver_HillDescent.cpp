...
std::unique_ptr<SearchResults>
    Recursive_Hill_Descent(std::unique_ptr<Board> board, u_long depth,
                           u_long                    maxDepth,
                           steady_clock::time_point &clock_start)
{
    ...
    if (!board->Collisions(&boardCost))
    { // This board is a solution!
        solution->push_back(std::move(board));
        return std::move(std::unique_ptr<SearchResults>(new SearchResults(
            depth, maxDepth, std::move(solution), GetTime(clock_start))));
    }
    ...
    for (unsigned int i = 0; i < childBoards->size(); i++)
    {
        ...
        std::unique_ptr<SearchResults> searchResult =
            std::move(Recursive_Hill_Descent(std::move(childBoard), depth + 1,
                                             maxDepth, clock_start));
...
std::unique_ptr<SearchResults>
    Solver_HillDescent::Solve(u_char                    size,
                              steady_clock::time_point &clock_start)
{
...
std::string Solver_HillDescent::GetName() { return "Hill Descent"; }